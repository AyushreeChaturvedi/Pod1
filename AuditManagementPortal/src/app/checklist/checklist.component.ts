...
export class ChecklistComponent implements OnInit {
  ...
  getQuestions(){
   ...
   getList(auditVal:number){
     this.loadFlag=true;
     let temp:any;
     console.log(this.queSerUrl+"?auditType="+auditVal,this.headers);
    this.http.get(this.queSerUrl+"?auditType="+this.auditType,this.headers).subscribe(response=>{
  ...
  responseYes(i:number){
    ...
    this.response[i].questionId=this.quesList[i].questionNo;
